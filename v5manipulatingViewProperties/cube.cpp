...
void Cube::Update() {
    ...
    vector<vector<float>> tRx = {
        ...
        vector<float>{0, cos(thX), sin(thX), 0},
        vector<float>{0, -sin(thX), cos(thX), 0},
        vector<float>{0, 0, 0, 1} };

    vector<vector<float>> tRy = {
        vector<float>{cos(thY), 0, -sin(thY), 0},
        vector<float>{0, 1, 0, 0},
        vector<float>{sin(thY), 0, cos(thY), 0},
        vector<float>{0, 0, 0, 1} };

    vector<vector<float>> tRz = {
        vector<float>{cos(thZ), sin(thZ), 0, 0},
        vector<float>{-sin(thZ), cos(thZ), 0, 0},
...
void Cube::KeyPress() {
    if (IsKeyDown(KEY_A)) thX += 0.1;
    if (IsKeyDown(KEY_D)) thX -= 0.1;

    if (IsKeyDown(KEY_W)) thY += 0.1;
    if (IsKeyDown(KEY_S)) thY -= 0.1;

    if (IsKeyDown(KEY_Q)) thZ += 0.1;
    if (IsKeyDown(KEY_Z)) thZ -= 0.1;

    if (IsKeyPressed(KEY_R)) thX = thY = thZ = 0;
}
